document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateGasCalculation(leituraGas, valorM3Gas) {
        ...
        document.getElementById('gas-calculation').textContent = 
            `${leituraGas.toFixed(2)} × ${valorM3Gas.toFixed(2)} = R$ ${valorGas.toFixed(2)}`;
        
        document.getElementById('gas-total').textContent = `R$ ${valorGas.toFixed(2)}`;
    ...
    function updateWaterCalculation(leituraAguaFria, leituraAguaQuente) {
        ...
        document.getElementById('water-consumption').textContent = 
            `${leituraAguaFria.toFixed(4)} + ${leituraAguaQuente.toFixed(4)} = ${totalConsumoAgua.toFixed(4)} m³`;
        
        const custoAguaEsgoto = calcularCustoAguaEsgoto(totalConsumoAgua);
        
        let waterCalculationHTML = `<div>Tarifa Base: R$ ${TARIFAS_AGUA_ESGOTO.BASE.toFixed(2)}</div>`;
        
        for (const faixa of TARIFAS_AGUA_ESGOTO.FAIXAS) {
            if (totalConsumoAgua >= faixa.min) {
                const consumoNaFaixa = Math.min(totalConsumoAgua, faixa.max) - faixa.min + 1;
                const valorFaixa = consumoNaFaixa * faixa.valor;
                
                waterCalculationHTML += `<div>Faixa ${faixa.min}-${faixa.max}: ${consumoNaFaixa.toFixed(0)} × R$ ${faixa.valor.toFixed(2)} = R$ ${valorFaixa.toFixed(2)}</div>`;
            }
        }
        
        document.getElementById('water-calculation').innerHTML = waterCalculationHTML;
        document.getElementById('water-total').textContent = `R$ ${custoAguaEsgoto.total.toFixed(2)}`;
        
        return custoAguaEsgoto.total;
    }
    
    function updateHeatingCalculation(leituraAguaQuente, valorCaldeira, totalAguaQuente) {
        const totalTaxaFixa = valorCaldeira * 0.2 / 185;
        const valorPorMetroQuadradoAquecido = valorCaldeira * 0.8 / totalAguaQuente;
        const valorTotalAquecimentoAgua = valorPorMetroQuadradoAquecido * leituraAguaQuente;
        const totalAquecimentoAgua = valorTotalAquecimentoAgua + totalTaxaFixa;
        
        document.getElementById('fixed-rate-calculation').textContent = 
            `${valorCaldeira.toFixed(2)} × 0.2 ÷ 185 = R$ ${totalTaxaFixa.toFixed(2)}`;
        
        document.getElementById('rate-per-m3-calculation').textContent = 
            `${valorCaldeira.toFixed(2)} × 0.8 ÷ ${totalAguaQuente.toFixed(3)} = R$ ${valorPorMetroQuadradoAquecido.toFixed(2)}`;
        
        document.getElementById('variable-cost-calculation').textContent = 
            `${valorPorMetroQuadradoAquecido.toFixed(2)} × ${leituraAguaQuente.toFixed(4)} = R$ ${valorTotalAquecimentoAgua.toFixed(2)}`;
