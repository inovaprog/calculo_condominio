...
const LEITURA_AGUA_QUENTE = 10.1940
const LEITURA_AGUA_FRIA = 11.7480
...
function calcularCustoAguaEsgoto(consumoTotal) {
    ...
    for (const faixa of TARIFAS_AGUA_ESGOTO.FAIXAS) {
        if (consumoTotal >= faixa.min) {
            const consumoNaFaixa = Math.min(consumoTotal, faixa.max) - faixa.min + 1;
...
console.log(`Custo total água + esgoto: R$ ${custoAguaEsgoto.total.toFixed(2)}`)


// AQUECIMENTO DE ÁGUA
const totalTaxaFixa = VALOR_CALDEIRA * 0.2 / 185 // quantidade de apartamentos
const valorPorMetroQuadradoAquecido = VALOR_CALDEIRA * 0.8 / TOTAL_SOMA_AGUA_QUENTE
const valorTotalAquecimentoAgua = valorPorMetroQuadradoAquecido * LEITURA_AGUA_QUENTE
console.log(`Custo total varivavel aquecimento água: R$ ${valorTotalAquecimentoAgua.toFixed(2)}`)
...
console.log(`Custo total aquecimento água: R$ ${totalAquecimentoAgua.toFixed(2)}`)
